...
async function checkTermClosedByComponent(componentId) {
  const { data: comp } = await supabase
    .from('grading_components')
  ...
  const { data: activeTerm } = await supabase
    ...
    .eq('is_active', true)
    .maybeSingle();
  return !activeTerm || activeTerm.school_year !== assign.school_year || activeTerm.semester !== assign.semester;
}

async function checkTermClosedByActivity(activityId) {
  const { data: act } = await supabase
    ...
    .single();
  if (!act) return true;
  return checkTermClosedByComponent(act.component_id);
}
...
router.put('/:id', authorizeRole('teacher'), async (req, res) => {
  ...
  try {
    if (await checkTermClosedByActivity(req.params.id)) {
      return res.status(403).json({ message: 'This term is closed. No modifications allowed.' });
    }
