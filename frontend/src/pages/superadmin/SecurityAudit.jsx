...
const SecurityAudit = () => {
  ...
  const getRoleInfo = (userName) => {
    if (userName === 'Super Admin' || userName.toLowerCase().includes('admin')) return { role: 'ROOT', color: '#ef4444' };
    const user = users.find(u => u.full_name === userName);
    if (!user) return { role: 'SYSTEM', color: '#3b82f6' };
    switch (user.system_role) {
      case 'superadmin': return { role: 'SUPER ADMIN', color: '#ef4444' };
      case 'admin': return { role: 'ADMIN', color: '#8b5cf6' };
  ...
  const dynamicRows = logs.map(log => {
    ...
    return { id: log.id, ts, user: log.user_name, role: roleInfo.role, roleColor: roleInfo.color, action: formatAction(log.action), desc: log.details || 'No description provided', count: 1, critical };
  });
  ...
  const getPageNumbers = () => {
    ...
    else { if (page === 1) pages = [1, 2, 3]; else if (page === totalPages) pages = [totalPages - 2, totalPages - 1, totalPages]; else pages = [page - 1, page, page + 1]; }
    return pages;
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-[#e5e0d5]">
        <div>
          ...
          <p className="text-xs sm:text-sm mt-0.5 text-gray-500">Real-time surveillance of system activity, access events, and security posture.</p>
        </div>
        ...
        <div>
          <div className="rounded-xl p-5 shadow-sm" style={{ background: '#fff', border: '1px solid #e5e0d5' }}>
            ...
            <div className="table-responsive"><table className="w-full text-xs min-w-[800px]">
              <thead><tr className="border-b" style={{ borderColor: '#f0ede6' }}>{['Timestamp', 'User', 'Role', 'Action', 'Description', '#'].map((h) => (<th key={h} className="text-left pb-2 pr-3 font-semibold text-gray-400 text-[10px] uppercase tracking-wide">{h}</th>))}</tr></thead>
              <tbody>
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t text-xs" style={{ borderColor: '#f0ede6' }}>
              <span className="text-gray-400">Showing {Math.min((page - 1) * ROWS_PER_PAGE + 1, dynamicRows.length)} to {Math.min(page * ROWS_PER_PAGE, dynamicRows.length)} of {dynamicRows.length.toLocaleString()} entries</span>
              <div className="flex items-center gap-1">
                ...
                {getPageNumbers().map((p) => (<button key={p} onClick={() => setPage(p)} className="w-7 h-7 rounded font-semibold" style={page === p ? { background: '#1a2233', color: '#fff' } : { background: '#f0ede6', color: '#6b7280' }}>{p}</button>))}
