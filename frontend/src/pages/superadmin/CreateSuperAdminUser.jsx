    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
      <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4 border border-gray-100 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-end gap-3 mt-6">
          ...
          <button type="button" onClick={handleSubmit} disabled={loading} className="px-4 py-2 text-sm font-bold text-white rounded-lg transition-colors shadow-md disabled:opacity-50 flex items-center gap-2" style={{ background: '#0F4A96' }}><UserPlus size={16} />{loading ? 'Creating...' : 'Create Admin'}</button>
        </div>
      </div>
    </div>
  );
