...
const GlobalAnalytics = () => {
      ...
      await Promise.all([
        (async () => {
          try {
            ...
            if (res.ok) { const data = await res.json(); setStaffUsers(data); }
  ...
  const handlePostNotification = (e) => {
    ...
    setNotification({ title: '', content: '', urgency: 'Not Urgent', audience: ['Admins', 'Teachers'] });
  ...
  <div className="space-y-6 max-w-7xl mx-auto">
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-[#e5e0d5]">
      <div>
        ...
        <p className="text-xs sm:text-sm mt-0.5 text-gray-500">Comprehensive Institutional performance oversight and trend forecasting.</p>
      </div>
    </div>

    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
      <MetricCard title="Total Accounts" value={staffUsers.length} icon={Users} color="#f5a623" subtitle="Registered members" />
      <MetricCard title="Faculty Members" value={totals.teachers} icon={BookOpen} color="#22c55e" subtitle="Teachers" />
      <MetricCard title="Admin Accounts" value={totals.admins} icon={ShieldCheck} color="#3b82f6" subtitle="Admins" />
      <MetricCard title="Audit logs" value={activityLog.length} icon={Activity} color="#a855f7" subtitle="Events logged" />
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="lg:col-span-2 flex flex-col gap-5">
        <div className="rounded-xl p-5 shadow-sm" style={{ background: '#fff', border: '1px solid #e5e0d5' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold" style={{ color: '#f5a623' }}>Department User Overview</h2>
              ...
              <table className="w-full text-xs min-w-[500px]">
                <thead>
                  <tr>
                    ...
                    {[{ label: 'Teachers', icon: BookOpen, color: '#22c55e' }, { label: 'Admins', icon: ShieldCheck, color: '#f97316' }].map(({ label, icon: Icon, color }) => (
                      <th key={label} className="pb-3 px-2 text-center">
                        <div className="flex flex-col items-center gap-1"><Icon size={14} style={{ color }} /><span className="font-medium text-gray-400">{label}</span></div>
                      </th>
                    ))}
        ...
        <div className="rounded-xl p-4 sm:p-5 shadow-sm" style={{ background: '#fff', border: '1px solid #e5e0d5' }}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
            <h2 className="text-sm sm:text-base font-bold" style={{ color: '#f5a623' }}>System Activities</h2>
            <div className="flex items-center gap-2">
              <Filter size={14} className="text-gray-400" />
              <select value={activityFilter} onChange={(e) => setActivityFilter(e.target.value)} className="text-xs px-2 py-1.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f5a623] bg-[#fbf8f1] border-[#e5e0d5] text-gray-700">
      ...
      <div className="flex flex-col gap-5">
        <div className="rounded-xl p-5 shadow-sm h-full" style={{ background: '#fff', border: '1px solid #e5e0d5' }}>
          <div className="flex items-center gap-2 mb-4"><Bell size={18} style={{ color: '#f5a623' }} /><h2 className="text-sm font-bold" style={{ color: '#1a2233' }}>Broadcast Notification</h2></div>
          <p className="text-[11px] mb-5 text-gray-500">Send an update or alert to Users (Teachers, Admins).</p>
          <form onSubmit={handlePostNotification} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 p-3 border rounded-lg bg-[#faf9f6] border-[#e5e0d5]">
                {['Admins', 'Teachers'].map((role) => (
                  <label key={role} className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-gray-700">
                    <input type="checkbox" checked={notification.audience.includes(role)} onChange={(e) => { let newAudience = [...notification.audience]; if (e.target.checked) newAudience.push(role); else newAudience = newAudience.filter(item => item !== role); setNotification({ ...notification, audience: newAudience }); }} className="rounded border-[#e5e0d5] text-[#f5a623] focus:ring-[#f5a623] w-4 h-4 cursor-pointer" />
                    {role}
                  </label>
                ))}
              </div>
            ...
            <div>
              ...
              <input type="text" placeholder="Notification Title" value={notification.title} onChange={(e) => setNotification({ ...notification, title: e.target.value })} className="w-full text-sm px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f5a623] bg-white border-[#e5e0d5]" required />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Message</label>
              <textarea placeholder="Write your message here..." value={notification.content} onChange={(e) => setNotification({ ...notification, content: e.target.value })} className="w-full text-sm px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#f5a623] bg-white border-[#e5e0d5] resize-none h-24" required />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">Urgency Level</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="urgency" value="Not Urgent" checked={notification.urgency === 'Not Urgent'} onChange={(e) => setNotification({ ...notification, urgency: e.target.value })} className="accent-[#f5a623]" /><span className="text-xs text-gray-700">Not Urgent</span></label>
                <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="urgency" value="Urgent" checked={notification.urgency === 'Urgent'} onChange={(e) => setNotification({ ...notification, urgency: e.target.value })} className="accent-red-500" /><span className="text-xs text-red-600 font-semibold flex items-center gap-1"><AlertTriangle size={12} /> Urgent</span></label>
              </div>
            </div>
            <button type="submit" className="w-full mt-2 py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2 transition-opacity hover:opacity-90 disabled:opacity-50" style={{ background: '#1a2233' }} disabled={!notification.title || !notification.content || notification.audience.length === 0}><Send size={14} /> POST NOTIFICATION</button>
