...
import { LayoutDashboard, ShieldCheck, Users, Building2, Settings, X, User, LogOut } from 'lucide-react';
...
const SuperAdminSidebar = ({ isOpen, onClose }) => {
  ...
  const navItems = [
    ...
    { name: 'Departments & Courses', path: '/superadmin/departments', icon: Building2 },
    { name: 'Admin Management', path: '/superadmin/users', icon: Users },
    ...
    <>
      ...
      >
        ...
        <div className="p-6 pb-8">
          <h1 className="font-bold text-md leading-tight uppercase tracking-wider">Super Admin Portal</h1>
        </div>
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  isActive
                    ? 'bg-[#0a316a] text-gold font-medium border-l-4 border-gold'
                    : ' hover:bg-[#0a316a] hover:text-white border-l-4 border-transparent'
                )}
              >
                <item.icon size={20} className={isActive ? 'text-gold' : 'text-white'} />
                {item.name}
              </Link>
            );
          })}
